...
from my_models.tools.metrics import metric
...
class Block_Rep(Patch_Basic):
    def __init__(self, args):
        super(Block_Rep, self).__init__(args)
    ...
    def patch_Sequence(self, x_in, ori_data, mask_ratio):
        ...
        unf_masked = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, D))
        ori_masked = torch.gather(ori_k, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, D))
        ...
        mask = torch.ones([bs, L]).to(self.device)
        mask[:, :len_keep] = 0
    ...
    def block_masking(self, x_block, mask_ratio):
        ...
        return x_enc_masked, block_masked_index
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_ori, batch_y_ori, ori_data) in enumerate(vali_loader):
                ...
                outputs = self.model(x_patch_masked)
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'PreTrain_checkpoint.pth')))
        preds = []
        trues = []
