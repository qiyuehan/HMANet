...
class deformable_LKA(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = DeformConv(dim, kernel_size=(3,3), padding=1, groups=dim)
...
class deformable_LKA_Attention(nn.Module):
    ...
    def forward(self, x):
        shorcut = x.clone()
        x = self.proj_1(x)
        x = self.activation(x) #[1, 384, 14, 14]
...
if __name__ == '__main__':
    # input = torch.rand(1, 96, 56, 56)
    input = torch.zeros(32, 44,8,12)
    # lka_layer_exp = deformable_LKA_Attention_experimental(d_model=96).cuda(0)
    lka_layer = deformable_LKA_Attention(d_model=44)
