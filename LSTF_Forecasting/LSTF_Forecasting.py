...
def main():
    ...
    parser.add_argument('--checkpoint', type=str,
                        default=r'/Users/qiyuehan/Desktop/HMANet/checkpoint/',
                        help='model path')
    # forecasting task
    parser.add_argument('--seq_len', type=int, default=768, help='input sequence length=seq_len-pred_len')
    parser.add_argument('--pre_train', type=int, default=960, help='input sequence length=seq_len-pred_len')
    parser.add_argument('--pred_len', type=int, default=96, help='prediction sequence length')
    parser.add_argument('--fea_dim', type=int, default=8, help='output dim')
    ...
    # save result
    parser.add_argument('--res_path', type=str, default=r'./chechpoint/res.csv', help='root path of the data file')
    ...
    if args.features == 'MS':
        df_ori = pd.read_csv(filePath, header=0, usecols=[args.target], encoding='latin-1').astype(float)
        data_length = np.random.randint(len(df_ori) - args.data_period)
        df_ori = df_ori[data_length: (data_length + args.data_period)]
        seq_in = df_ori
    else:
        ...
        print("Testset Length:", int(len(df_ori)))
    ...
    x_ori, y_ori = getDataMatrix(seq_in, args.features, args.seq_len, args.pred_len, args.stride)
    _, ori_data, _, _ = train_test_split(x_ori, y_ori, test_size=args.test_percent, random_state=1)
    norm_ori = norm_data(seq_in)  # (5000, 7)
    ...
    model_path = args.checkpoint + args.model_name + '.pth'
...
def get_representation(args, model_pre, batch_train, batch_train_ori, num_group):
    ...
    for i in range(len(batch_train)):
        ...
        pre_fea = model_pre(split_blocks, 0, 0)  # 32,44,8,12
        get_representation.append(pre_fea)

    f_dim = -1 if args.features == 'MS' else 0
    pre_rep = torch.cat(get_representation, dim=0)  # 249 * [32, 960, 8]
...
def test_representation(args, model_pre, batch_train, batch_train_ori,  num_group):
    ...
    f_dim = -1 if args.features == 'MS' else 0
    pre_rep = torch.stack(get_representation, dim=0)  # 249 * [32, 960, 8]
    train_rep = pre_rep[:, :, :args.seq_len, f_dim:]
...
def train_valid(args, model_pre, batch_train, batch_train_ori, batch_tar_train, batch_tar_train_ori, batch_tar_test,
    ...
    while epoch_now < args.epochs:
        ...
        net = ELM_L(in_dim, n_hidden_dim).to(device)
        ...
        # representation    (args, model_pre, batch_train, batch_train_ori, batch_tar_train,  num_group
        ...
        R2 = metrics.r2_score(batch_tar_test_ori, test_output.cpu())
    ...
    print("Training Result-> Best MSE:", bmse, "Best MAE:", bmae, 'Best Hidden:', best_hidden)
...
def test_Result(args, net, model_pre, b_beta, batch_test, batch_test_ori, batch_tar_test, batch_tar_test_ori,
    ...
    test_inputs = test_representation(args, model_pre, batch_test, batch_test_ori, batch_tar_test, num_group)
    # 验证
    for i in range(len(test_inputs)):
        ...
        R2 = metrics.r2_score(target, output.cpu())
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        # torch.save(model, path + '/' + 'beta.pth')
